import {
  appealVerdict,
  CLIENT_AI_PROVIDER_HEADER,
  CLIENT_OPENROUTER_KEY_HEADER,
  getPublicAiStatus,
  judgeObject,
...
async function runPost(request, operation) {
  try {
    ...
    const clientCredential = parseClientAiCredential({
      authorization: request.headers.get(CLIENT_OPENROUTER_KEY_HEADER),
      provider: request.headers.get(CLIENT_AI_PROVIDER_HEADER),
    });
    ...
    try {
      ...
      try {
        const payload = await operation(body, controller.signal, { clientCredential });
...
export function handleStatus() {
  return jsonResponse(200, getPublicAiStatus());
}
